...
import addrCoordinate from '../helpers/location.js';
...
let DUMMY_PLACES = [
	{
		...
		description:
			'The most famous sky scrapers in the world!',
		location: {
			lat: 40.7484474,
			lng: -73.9871516,
		},
...
const placeById = (req, res, next) => {
	...
	// if no place
	if (!place) {
		throw new RaiseError('No matching place found.', 404);
	}
...
const placesByUser = (req, res, next) => {
	...
	if (!places || places.length === 0) {
		return next(
			new RaiseError('No matching place found.', 404)
		);
	}
...
const deletePlace = (req, res, next) => {
	...
	res
		.status(200)
		.json({ message: 'Place has been deleted.' });
};
